...
const { dateFormat, timeFormat } = require('./UtilController');
...
async function vendaPdv(file) {
    try {
        ...
        for (const linha of linhas) {
            ...
            } else if (tipoRegistro === 'DOC') {
                if (tipoAtual !== 'DOC') {
                    tipoAtual = 'DOC';
                    registrosAtuais = [];
                    registrosDOC.push(registrosAtuais);
                }
                registrosAtuais.push(campo);
        ...
        return { registrosINI, registrosDIT, registrosDOC, registrosFIM };
    } catch (err) {
        console.error(err);
        throw err;
    }
                ...
                try {
                    ...
                    await new Promise((resolve) => {
                        transaction.rollback(function () {
                            ...
                            resolve(); // Resolva a promise após o rollback ser concluído
                        });
                    });
                }
            });
    ...
    try {
        ...
        for (const file of files) {
            if (file.indexOf('.') !== 0 && file.slice(-4) === '.djm') {
                ...
                const { registrosINI, registrosDIT, registrosDOC, registrosFIM } = await vendaPdv(`${dirfiles}/${file}`);

                // Processar registros DIT
                for (const registros of registrosDIT) {
                    await finalizarProcessamento(registrosINI, registros, registrosFIM);
                }

                // Processar registros DOC
                for (const registros of registrosDOC) {
                    await finalizarProcessamento(registrosINI, registros, registrosFIM);
                }
            }
        }
